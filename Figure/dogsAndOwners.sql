...
-- S?lectionnez tous les chiens avec leur nom, leur race et leur poids.

SELECT name, breed, weight
FROM Dogs;

-- Listez tous les propri?taires (pr?nom et nom).

 SELECT first_name, last_name
 FROM People;

-- R?cup?rez les chiens qui n'ont pas de ma?tre.

 SELECT *
 FROM Dogs
 WHERE owner_id IS NULL;

-- S?lectionnez tous les chiens de race "Labrador".
...
-- R?cup?rez les ma?tres qui poss?dent un chien pesant plus de 20 kg.
...
--- Niveau 3 : LEFT JOIN
-- Affichez tous les propri?taires et les chiens qu'ils poss?dent, y compris les propri?taires sans chien.

 SELECT People.first_name, People.last_name, Dogs.name AS DogName
 FROM People
 LEFT JOIN Dogs ON People.id = Dogs.owner_id;

-- Listez tous les chiens, avec leurs ma?tres s'ils en ont, sinon affichez "No Owner".
...
-- R?cup?rez tous les chiens et tous les ma?tres, m?me ceux sans correspondance.
...
-- Affichez les chiens dont le poids est sup?rieur ? 10 kg mais inf?rieur ? 30 kg.
	
 SELECT *
 FROM Dogs
 WHERE weight > 10 AND weight < 30;
	
-- R?cup?rez les chiens de ma?tres habitant dans la ville "123 Main St".
...
-- Affichez le nombre de chiens pour chaque ma?tre.
...
-- Calculez le poids total des chiens appartenant ? chaque ma?tre.
...
-- R?cup?rez les ma?tres qui poss?dent le chien le plus lourd.
...
-- Affichez les chiens qui ont un ma?tre dont l'?ge est sup?rieur ? 40 ans.
...
-- Listez les ma?tres n'ayant pas de chien.
...
-- Listez tous les ma?tres qui poss?dent au moins deux chiens.
...
-- R?cup?rez une liste combin?e de chiens sans ma?tres et de ma?tres sans chiens.
...
-- Affichez le ma?tre et ses chien associ?s avec somme de leur tailles respectives (taille du ma?tre et des chiens).
